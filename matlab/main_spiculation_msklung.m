...
%     end
    ...
    %% input & ouput files
    label_file = [experiment_path '/' pid '/' filename '-label.nrrd'];
    ...
    %% attachment vertices
    attachment = find_attached_vertices(attached_label_filename, s);
    ...
    dlmwrite([output_experiment_path '/parameters/' filename '_nd.txt'],nd);
    dlmwrite([output_experiment_path '/parameters/' filename '_attached.txt'], attachment_indices);
    dlmwrite([output_experiment_path '/parameters/' filename '_apex.txt'],[spikes.apex]');

    %% save apex point of spikes and area distortion for slicer
    fid = fopen([output_experiment_path '/parameters/' filename '.fcsv'],'w');
    ...
    f = table({filename}, {pid},{nid},num_spikes,sum(spiculation),sum(lobulation), sum(attached), mean(attachment), ...
     ...
     writetable(f, [output_experiment_path '/parameters/' filename '_features.txt']);
...
%     plot3(l_loc(:,1),l_loc(:,2),l_loc(:,3),'y*')
    ...
    savefig([output_experiment_path '/spiculation/' filename '_sphere_param.fig'])
    ...
    print([output_experiment_path '/spiculation/' filename '_sphere_param.png'],'-dpng','-r300')
    clear colors
    close all
    toc
end
