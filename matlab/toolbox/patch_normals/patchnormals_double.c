...
/* 
 * 
 * function [Nx,Ny,Nz,FNx,FNy,FNz]=patchnormals_double(Fa,Fb,Fc,Vx,Vy,Vz)
 *
*/
...
void mexFunction( int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[] )
{
    ...
    /* All outputs, Vertex and Face Normals */
   ...
   if(nrhs!=6) {
     ...
     mexErrMsgTxt("6 outputs are required");
   }
   ...
   FacesNA[0]=FacesN;
   plhs[3] = mxCreateNumericArray(1, FacesNA, mxDOUBLE_CLASS, mxREAL);
   plhs[4] = mxCreateNumericArray(1, FacesNA, mxDOUBLE_CLASS, mxREAL);
   plhs[5] = mxCreateNumericArray(1, FacesNA, mxDOUBLE_CLASS, mxREAL);
   
   NormalsX = mxGetPr(plhs[0]);
   NormalsY = mxGetPr(plhs[1]);
   NormalsZ = mxGetPr(plhs[2]);
   
   FaceNormalsX = mxGetPr(plhs[3]);
   FaceNormalsY = mxGetPr(plhs[4]);
   FaceNormalsZ = mxGetPr(plhs[5]);
