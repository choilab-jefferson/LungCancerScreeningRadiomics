function [N, FN, flip]=patchnormals(FV)
...
[Nx,Ny,Nz,FNx,FNy,FNz]=patchnormals_double(double(FV.faces(:,1)),double(FV.faces(:,2)),double(FV.faces(:,3)),double(FV.vertices(:,1)),double(FV.vertices(:,2)),double(FV.vertices(:,3)));
...
FN=zeros(length(FNx),3);
FN(:,1)=FNx;
FN(:,2)=FNy;
FN(:,3)=FNz;

%% Check vector direction to make it to outside
if sizev(1) > 100
    shuffled = randperm(sizev(1));
    selected = shuffled(1:100);
    sizes = 100;
else
    selected = 1:sizev(1);
    sizes = sizev(1);
...
ce = mean(FV.vertices(selected,:));
l1 = sum((FV.vertices(selected,:)+N(selected,:)-repmat(ce,[sizes,1])).^2,2);
l2 = sum((FV.vertices(selected,:)-repmat(ce,[sizes,1])).^2,2);
% check direction
if sum((l1-l2)<0) > 55 % greater than 55%
    N = -N;
    FN = -FN;
    flip = true;
end
