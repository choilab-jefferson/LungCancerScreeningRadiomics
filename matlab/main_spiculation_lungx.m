...
for idx = 1:size(pid_list,2)
    ...
    try
        [spikes_table, f] = spiculation_quantification(output_experiment_path, obj_filename, sph_map_filename, n_info);
    ...
    try
        file_prefix = [experiment_path '/' pid '/' pid '_CT_' nid '-seg'];
        voxelize_meshes(output_experiment_path, file_prefix)
    catch
        continue
    end
...
%     plot3(l_loc(:,1),l_loc(:,2),l_loc(:,3),'y*')
    ...
    title_text = [pid , nid,' Volume:',num2str(volume/1000,2),'cc, D:',num2str(esr*2),'mm, #Spikes:',num2str(num_spikes),', #Spiculation:',num2str(sum(spiculation)),', Roundness:',num2str(roundness,2)];
